        ...
        child: Column(
          children: [
            ...
            Table(
              ...
              children: [
                _buildTableRow('Name:', order.user!.name ?? ''),
                _buildTableRow('Email:', order.user!.email ?? ''),
                _buildTableRow('Phone:', order.user!.phone ?? ''),
                _buildTableRow('Ordered date:', formattedDate),
              ],
            ),
          ],
        ),
        ...
        child: Column(
          children: [
            ...
            Table(
              ...
              children: [
                _buildTableRow('Field name:', order.field!.name!),
                _buildTableRow(
                    'Date:', FormatUtil.formatISOtoDate(order.date!) ?? ''),
                _buildTableRow('Time:',
                    '${FormatUtil.formatISOtoTime(order.startTime!)} - ${FormatUtil.formatISOtoTime(order.endTime!)}'),
                _buildTableRow(
            ...
            Container(
              ...
              child: ElevatedButton(
                  onPressed: () {
                    Get.toNamed(RoutePaths.fieldBooking, parameters: {
                      'fieldID': order.field!.sId!,
                      'sellerID': order.seller!.sId!
                    });
                  },
                  style: ElevatedButton.styleFrom(),
                  child: const Text('View field')),
            )
          ],
        ),
      ),
